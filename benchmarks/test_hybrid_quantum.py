...
from pimst.improved.sino.hybrid_quantum_solver import HybridQuantumSolver
...
print("="*70)
print("  🌟 HYBRID QUANTUM - Mejor de ambos mundos")
print("="*70)
...
for name, optimal in instances.items():
    ...
    print(f"\n🎯 Gap final: {gap:+.2f}%")
    print(f"   Smart: {meta['smart_cost']:.2f}")
    print(f"   Quantum: {meta['quantum_cost']:.2f}")
    print(f"   Ganador: {meta['winner']}")
    
    results.append({'name': name, 'gap': gap})

print("\n" + "="*70)
print("  📊 RESUMEN HYBRID")
print("="*70)
...
print(f"   Hybrid Quantum: {np.mean(gaps):.2f}%")

if np.mean(gaps) < 3.40:
    improvement = ((3.40 - np.mean(gaps)) / 3.40) * 100
    print(f"\n🏆 ¡NUEVO RÉCORD! {improvement:.1f}% mejor que Quantum")
elif np.mean(gaps) < 4.31:
    print(f"\n✅ Mejor que Smart Quantum solo")
else:
